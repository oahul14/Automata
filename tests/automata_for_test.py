...
def life(initial_state, nsteps, periodic=False):
    ...
    def counter(i, j, field):
        ...
        :param field: The copied matrix
        :return: The sum of True neighbours
        '''

        left = (field[i-1, j]).astype(int)
        lefttop = field[i-1, j-1]
        top = field[i, j-1]
        topright = field[i+1, j-1]
        right = field[i+1, j]
        rightbot = field[i+1, j+1]
        bot = field[i, j+1]
        botleft = field[i-1, j+1]
...
def life_generic(matrix, initial_state, nsteps, environment, fertility):
    ...
    def check_rule(test_ary, env, fert, status='life'):
        if status == 'life':
            return np.isin(test_ary, list(env))
        return np.isin(test_ary, list(fert))
    ...
    while i < nsteps:
        ...
        true_new_status = check_rule(sum_true, environment, fertility, status='life')
        false_new_status = check_rule(sum_false, environment, fertility, status='dead')

        mesh[row_true], mesh[row_false] = true_new_status, false_new_status

...
def sandpile(initial_state):
    ...
    mesh = np.pad(mesh, pad_width=1, mode='constant', constant_values=0)
    ...
    while np.max(mesh) >= 4:
        ...
        row, col = np.where(mesh >= 4)[0], np.where(mesh >= 4)[1]
...
def lifetri(initial_state, nsteps, periodic=False):
    ...
    def false_border(mesh):
        '''
        Create a border filled with zero for the mesh
        :param mesh: the input mesh with random boolean
        :param n: the size of the square mesh
            :return: the square mesh with an outer boundary filled with False
        '''
        mesh[0:2, :] = False
    ...
    def periodic_border(mesh):
        mesh[2:-2, 0] = mesh[2:-2, -4]
        mesh[2:-2, 1] = mesh[2:-2, -3]

        mesh[2:-2, -2] = mesh[2:-2, 2]
        mesh[2:-2, -1] = mesh[2:-2, 3]

        mesh[0, 2:-2] = mesh[-4, 2:-2]
        mesh[1, 2:-2] = mesh[-3, 2:-2]

        mesh[-2, 2:-2] = mesh[2, 2:-2]
        mesh[-1, 2:-2] = mesh[3, 2:-2]

        mesh[0:2, 0:2] = mesh[-4:-2, -4:-2]
        mesh[0:2, -2:] = mesh[-4:-2:, 2:4]
        mesh[-2:, 0:2] = mesh[2:4, -4:-2]
        mesh[-2:, -2:] = mesh[2:4, 2:4]
        return mesh
    ...
    mesh = np.pad(initial_state, pad_width=2, mode='constant', constant_values=False)
    ...
    while i < nsteps:
        ...
        mesh_true = mesh.copy()
        row_true = np.where(mesh_true[2:-2, 2:-2] == 1)[0]+2
        col_true = np.where(mesh_true[2:-2, 2:-2] == 1)[1]+2
